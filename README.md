...
- Designed and implemented a three-rule asymmetric exit system to manage downside risk on intraday event trades: a hard stop that closes any position losing more than 2 basis points within the first 30 seconds of entry (protecting against immediate adverse price reactions to releases), a high-watermark trail exit that locks in gains by closing if the trade retraces 60% from its peak return, and a time-based exit that terminates all positions at a predefined horizon — preventing overnight exposure and ensuring trades remain within the event signal window.

- Developed a performance attribution and analytics module decomposing signal hit rate, gross return, and expectancy by instrument, event tier, z-score bucket, and VIX regime; quantified basis risk by measuring co-hit rates across instrument pairs and detecting conflicting signals — cases where the same macro release generates directionally opposite trade signals on correlated instruments — to identify where the portfolio's diversification assumption breaks down under real market conditions; added PCA-based factor concentration scoring and a five-criterion Go/No-Go gate to govern the transition from backtesting to paper trading.
