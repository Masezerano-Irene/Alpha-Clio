from __future__ import annotations

import argparse
import sys
from pathlib import Path

import pandas as pd

_REPO_ROOT = Path(__file__).resolve().parents[1]
if str(_REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(_REPO_ROOT))

from mini_hedge.config import PROJECT_ROOT


def _normalize(df: pd.DataFrame) -> pd.DataFrame:
    if "release_date" in df.columns and "date" not in df.columns:
        df = df.rename(columns={"release_date": "date"})
    if "date" not in df.columns:
        raise ValueError("Input must include a `date` (or `release_date`) column.")
    df["date"] = pd.to_datetime(df["date"]).dt.normalize()

    if "expected_change_pp" not in df.columns:
        if "p_cut25" in df.columns and "p_hike25" in df.columns:
            df["expected_change_pp"] = (-0.25 * df["p_cut25"]) + (0.25 * df["p_hike25"])
        else:
            raise ValueError(
                "Input must include `expected_change_pp` OR both `p_cut25` and `p_hike25`."
            )

    cols = [c for c in ["date", "p_hold", "p_cut25", "p_hike25", "expected_change_pp"] if c in df.columns]
    return df[cols].sort_values("date").reset_index(drop=True)


def main() -> None:
    ap = argparse.ArgumentParser(description="Merge a vendor/historical FOMC probabilities CSV into data/fomc_probabilities_history.csv")
    ap.add_argument(
        "input_csv",
        nargs="?",
        default=str(PROJECT_ROOT / "data" / "fomc_probabilities.csv"),
        help="Input CSV path (default: data/fomc_probabilities.csv)",
    )
    ap.add_argument(
        "--dest",
        default=str(PROJECT_ROOT / "data" / "fomc_probabilities_history.csv"),
        help="Destination path (default: data/fomc_probabilities_history.csv)",
    )
    args = ap.parse_args()

    input_path = Path(args.input_csv)
    if not input_path.is_absolute():
        input_path = PROJECT_ROOT / input_path
    if not input_path.exists():
        raise FileNotFoundError(
            f"Input CSV not found: {input_path}\n\n"
            "Provide a path explicitly, e.g.\n"
            "  python scripts/ingest_fomc_probabilities_history.py path/to/file.csv\n\n"
            "Or create data/fomc_probabilities.csv with columns like `date` and `expected_change_pp` "
            "(or `p_cut25` and `p_hike25`)."
        )

    inc = _normalize(pd.read_csv(input_path))
    dest = Path(args.dest)
    dest.parent.mkdir(parents=True, exist_ok=True)

    if dest.exists():
        cur = _normalize(pd.read_csv(dest))
        out = pd.concat([cur, inc], ignore_index=True)
    else:
        out = inc

    out = out.sort_values("date").drop_duplicates(subset=["date"], keep="last").reset_index(drop=True)
    out.to_csv(dest, index=False)
    print(f"Wrote {len(out):,} rows to {dest}")


if __name__ == "__main__":
    main()
